...
const FC_SOURCE_LABELS: Record<FinalConclusion["source"], string> = {
  ...
  surviving_branch:      "생존 Branch 결론",
  structural_consensus:  "구조적 합의",
};
...
export function AnalysisModal({ goal, analysis, topic, history, onClose }: Props) {
  ...
  // 세그먼트 탭은 반드시 해당 segment의 isolated analysis만 사용 — fallback 없음
  ...
  // 헤더는 통합 탭이면 전체 analysis, 세그먼트 탭이면 해당 segment analysis
    ...
    <div className="am-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="am-modal" role="dialog" aria-modal="true">
        ...
        <div className="am-body">
          ...
          {/* 통합 분析 탭: CognitiveFramework 최상단 → FinalResolution → QuestionEvolution → MetaEvolution */}
...
// ─── AnalysisBody ────────────────────────────────────────────────
// 분석 섹션 본문을 독립 컴포넌트로 추출 — 세그먼트 탭 전환 시 재사용
  ...
  return (
    <>
      ...
      {synthesis && synthesis.unresolvedKeywords.length > 0 && (
        <Section title="미해결 키워드" accent="deadlock">
          <div className="am-shared-concepts">
      ...
      {/* 수렴 포화 감지 */}
      {saturation && (
        <Section title="수렴 포화 감지" accent={saturation.saturated ? "saturation" : undefined}>
      ...
      {/* 교착 분석 */}
      {deadlockAnalysis && (
        <Section title="교착 분석" accent="deadlock">
          ...
          <div className="am-novelty-chart">
            ...
            {stagnationRounds > 0 && (
              <span className="am-novelty-stagnation">
                마지막 {stagnationRounds}라운드 저-novelty
              </span>
            )}
          </div>
        </Section>
      ...
      {convergenceHistory && convergenceHistory.length > 1 && (
        <Section title="Actor 수렴 추이 (Jaccard)">
      ...
      {unresolvedConflicts && unresolvedConflicts.length > 0 && (
        <Section title="미해결 충돌 축" accent="deadlock">
          {unresolvedConflicts.map((c, i) => (
            <div key={i} className="am-conflict-row">
              <div className="am-conflict-dim">축: {c.dimension}</div>
              {c.positions.map(pos => (
      ...
      {/* 구조 수렴 / Semantic Loop / Concept Gravity */}
...
function ConvergenceFreezeSection({ cf }: { cf: ConvergenceFreezeAnalysis }) {
  ...
  return (
    <Section title="수렴 동결 분석" accent="convergence">
        ...
        {cf.frozen ? (
          <>
            ...
            {cf.frozenAtRevisionId !== undefined && (
              <span className="am-cf-frozen-at">revision #{cf.frozenAtRevisionId}부터 동결</span>
            )}
          </>
        ) : (
          <span className="am-cf-freeze-badge am-cf-active">수렴 진행 중</span>
        )}
        <span className="am-cf-entropy">
          Entropy {(cf.argumentEntropy * 100).toFixed(0)}%
      ...
      {scores.length > 0 && (
        <div className="am-cf-novelty-section">
          <div className="am-cf-sub-label">Revision별 Novelty (새 논거 비율)</div>
          <div className="am-cf-graph">
            {scores.map((s, i) => {
              ...
              return (
                <div key={s.revisionId} className="am-cf-bar-col" title={`#${s.revisionId}: novelty ${(s.novelty * 100).toFixed(0)}%`}>
          ...
          <div className="am-cf-graph-legend">
            <span className="am-cf-leg am-cf-leg-meaningful">★ 마지막 의미 진화</span>
            <span className="am-cf-leg am-cf-leg-freeze">↓ 동결 시작</span>
            <span className="am-cf-leg am-cf-leg-collapse">⊗ Entropy 붕괴</span>
          </div>
        </div>
      )}
      ...
      <div className="am-cf-stats">
        <div className="am-cf-stat-item">
          <span className="am-cf-stat-label">Argument Entropy</span>
        ...
        {cf.lastMeaningfulRevisionId !== undefined && (
          <div className="am-cf-last-meaningful">
            <span className="am-cf-stat-label">마지막 의미 진화</span>
            <span className="am-cf-rev-id">revision #{cf.lastMeaningfulRevisionId}</span>
          </div>
        )}
      </div>
    </Section>
...
function BranchSurvivalSection({ bs }: { bs: BranchSurvivalAnalysis }) {
  return (
    <Section title="생존한 논리 Branch" accent="convergence">
      ...
      {bs.branches.map(branch => (
        <div key={branch.id} className={`am-bs-branch ${branch.dominant ? "am-bs-dominant" : ""}`}>
          <div className="am-bs-branch-header">
            {branch.dominant && <span className="am-bs-dominant-badge">지배 Branch</span>}
          ...
          <div className="am-bs-stats">
            ...
            <span className="am-bs-stat">양보 {branch.concedeDepth}회</span>
...
function EvolutionPressureSection({ ep }: { ep: EvolutionPressureAnalysis }) {
  ...
  return (
    <Section title="논리 진화 압력" accent="convergence">
      <div className="am-ep-root">
          ...
          {ep.semanticDecayActors.length > 0 && (
            <div className="am-ep-decay-actors">
              {ep.semanticDecayActors.map(a => (
                <span key={a} className={`am-actor-chip actor-${a} am-ep-decay-chip`}>{a} ↓반복</span>
              ))}
            </div>
          )}
        </div>
        ...
        {topMomentum.length > 0 && (
          <div className="am-ep-momentum">
            <div className="am-ep-sub-label">Actor 진화 기여도</div>
            {topMomentum.map(m => (
        ...
        {ep.innovationMoments.length > 0 && (
          <div className="am-ep-innovations">
            <div className="am-ep-sub-label">혁신 순간</div>
...
function ConvergenceFlowSection({ analysis }: { analysis: DiscussionAnalysis }) {
  ...
  return (
    <Section title="논리 수렴 과정" accent="convergence">
      <div className="am-cf-flow">
        ...
        {chain.length >= 3 && (
          <>
            ...
            <div className="am-cf-phase">
              <div className="am-cf-phase-label">논리 진화</div>
              {chain.map(node => (
    ...
    <div className="am-fc">
      ...
      {fc.basedOnRevisionIds.length > 0 && (
        <div className="am-fc-revids">
          <span className="am-label">기반 revision</span>
      ...
      {fc.source !== "selected_option" && synthesis && (
        <div className="am-fc-vs">
          <span className="am-label">selectedOption과의 차이</span>
    ...
    <div className="am-saturation">
      <div className="am-saturation-meta">
        <span className={`am-saturation-badge ${saturation.saturated ? "am-sat-active" : "am-sat-inactive"}`}>
          {saturation.saturated ? "포화 감지됨" : "포화 미감지"}
        </span>
      ...
      {recent5Avg !== null && (
        <div className="am-saturation-stat">
          <span className="am-label">최근 5라운드 평균 novelty</span>
      ...
      {saturation.repetitionClusters.length > 0 && (
        <div className="am-saturation-clusters">
          <div className="am-label" style={{ marginBottom: 6 }}>반복 semantic 클러스터</div>
